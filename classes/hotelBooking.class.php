...
private function roomBooking($date_start, $date_end, $room_id){
    ...
    $stmt = $this->connect()->prepare('SELECT * FROM HotelBookings WHERE (date_start <= ? AND date_end >= ?) OR (date_start <= ? AND date_end >= ?) AND room_id = ?');

    if($stmt->execute([$date_start, $date_start,$date_end, $date_end, $room_id])){
